...
export function Toolbar({ editor, className }: Props) {
  ...
  const [hiddenButtons, setHiddenButtons] = useState<ButtonName[]>([]);
    ...
    const doAdapt = () => {
      ...
      if (mainToolbar) {
        ...
        if (isOverflowing) {
          ...
          const tempHiddenButtons: ButtonName[] = [];

          buttonsArray.forEach((button, index) => {
            const btn = button as HTMLElement;
            if (!isFullyVisible(btn)) {
              tempHiddenButtons.push(button_name_list[index]!);
            } else {
          ...
          setHiddenButtons(tempHiddenButtons);
          setShowMore(true);
        } else {
          setShowMore(false);
        }
  ...
  const isButtonHidden = (buttonName: ButtonName) => {
